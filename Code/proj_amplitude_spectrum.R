...
data = prepare_training_data(result)

library(caret)


set.seed(12345)
s <- createDataPartition(y = data$class,p = 0.7,list = FALSE)
training_data <- data[s,]
test_data <- data[-s,]

# Determine how many components to keep
pca_result <- prcomp(training_data[, -ncol(training_data)], scale. = TRUE)
...
training_pca <- predict(pca_result, training_data[, -ncol(training_data)])[, 1:num_components]
training_pca_df <- as.data.frame(training_pca)
training_pca_df$label <- training_data$class
...
rf.model=randomForest(label~.,data=training_pca_df,mtry=105,importance=TRUE)
yhat.bag = predict(rf.model,newdata=test_pca_df, type="class")
table(yhat.bag,test_pca_df$label)
mean(yhat.bag==test_pca_df$label)

#p=105, sqrt(105) approx.=11
rf.model=randomForest(label~.,data=training_pca_df,mtry=11,importance=TRUE)
yhat.rf = predict(rf.model,newdata=test_pca_df, type="class")
table(yhat.rf,test_pca_df$label)
mean(yhat.rf!=test_pca_df$label)
importance(rf.model)
varImpPlot(rf.model)

# 1. Support Vector Machine (SVM)
svm.model <- svm(label ~ ., data = training_pca_df, kernel = "radial", 
...
# 3. Decision Tree
dt.model <- rpart(label ~ ., data = training_pca_df, method = "class")
...
label_mapping <- as.integer(factor(training_pca_df$label)) - 1
test_label_mapping <- as.integer(factor(test_pca_df$label)) - 1

# Prepare matrices for XGBoost
train_matrix <- as.matrix(training_pca_df[, -which(names(training_pca_df) == "label")])
...
params <- list(
  ...
  num_class = length(unique(training_pca_df$label)),
  eta = 0.3,
  max_depth = 6,
  subsample = 0.8,
...
class_levels <- levels(factor(training_pca_df$label))
...
# Compare all models
error_rates <- c(
  "Random Forest" = mean(yhat.rf != test_pca_df$label),
  "SVM" = svm_error_rate,
